      ...
      <div class="flex-element buttons_new_hazard">
        ...
        <div class="column column3 button-column">
          <router-link :to="{ name: 'ListView'}">
            <div class="button button-cancel" id="cancel">
              Abbrechen
            </div>
          </router-link>
          <router-link :to="{ name: 'ListView'}">
  ...
  methods: {
    ...
    addNewTextfield(TopicofnewTextfield) {
      ...
      divAdd.innerHTML = '<div class="flex_add">\n' +
          ...
          '<input class="input_customized add_new_input" type="text"' +
          'style="box-sizing: border-box; width: calc(300 * 0.063rem); height: calc(29 * 0.063rem); border-radius: calc(5 * 0.063rem); margin-left:calc(260 * 0.063rem); padding-left: calc(5 * 0.063rem); border: 1px solid #E1E5EB">' +
          '</div></div></div>';
...
[data-tooltip]::after {
  ...
  opacity: 0;
  transform: translateY(calc(-1% - 10px)) translateX(-1%) scale(0);
  transition: 0.25s transform;
  transform-origin: center center;
}

[data-tooltip]:hover::after, [data-tooltip]:hover::before, [data-tooltip2]:hover::after, [data-tooltip2]:hover::before {
  position: absolute;
  opacity: 1;
  visibility: visible;
...
[data-tooltip2]::after {
  ...
  padding: 10px;
  border-radius: 5px;
  font-size: calc(calc(var(--mainFontSize) + 1) * 0.063rem);
  visibility: hidden;
  transform: translateY(calc(-1% - 10px)) translateX(-1%) scale(0);
  transition: 0.25s transform;
  transform-origin: center center;
